...
int tree_parse(const void *data, size_t len, Tree *tree_out) {
    ...
    while (ptr < end && tree_out->count < MAX_TREE_ENTRIES) {
        ...
        // 1. Safely find the space character for the mode for required implementation
...
// directory prefix (prefix_len chars deep), build a tree and write it.
// Returns 0 on success, -1 on error.
static int write_tree_level(const IndexEntry *entries, int count,
    ...
    int i = 0;
    while (i < count) {
        // Path relative to current level (skip the prefix)
        const char *rel = entries[i].path + prefix_len;

        // Find if there's a slash — means it's in a subdirectory
        ...
        if (!slash) {
            ...
            // It's in a subdirectory. Figure out the subdirectory name.
            ...
            while (i < count) {
                ...
                if (!s) break; // no slash — file, not subdir
                size_t dlen = (size_t)(s - r);
                if (dlen != dir_name_len || strncmp(r, dir_name, dlen) != 0) break;
                i++;
            }

            // Recursively build the subtree for this subdirectory
            size_t new_prefix_len = prefix_len + dir_name_len + 1; // +1 for the '/'
            ObjectID sub_id;
            if (write_tree_level(entries + start, i - start,
                                  prefix, new_prefix_len, &sub_id) != 0)
                return -1;

            // Add an entry for this subdirectory
            TreeEntry *te = &tree.entries[tree.count++];
...
// Helper to sort index entries by path for the recursive builder
